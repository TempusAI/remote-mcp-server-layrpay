...
// Helper function to handle MCP requests (shared between HTTP and SSE transports)
async function handleMcpRequest(mcpRequest: any, env: Env): Promise<any> {
  console.log('Processing MCP request:', JSON.stringify(mcpRequest, null, 2));
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // Handle HTTP transport endpoint (standard MCP over HTTP)
    if (url.pathname === '/http') {
      ...
      try {
        ...
        console.log('HTTP transport - Received request body:', body);
    ...
    if (url.pathname === '/sse') {
      ...
      if (request.method === 'GET') {
        // Return proper SSE stream with keep-alive
        const { readable, writable } = new TransformStream();
        const writer = writable.getWriter();
        
        // Send initial SSE connection
        writer.write(new TextEncoder().encode('data: {"jsonrpc":"2.0","method":"notifications/initialized"}\\n\\n'));
      ...
      try {
        const body = await request.text();
        console.log('SSE transport - Received request body:', body);
        ...
        console.log('SSE transport - Sending response:', response);
